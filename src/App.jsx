...
function CharacterFlowNode({ data }) {
  return (
    <div className={data.selectedForConnect ? "flow-character-node selected" : "flow-character-node"}>
...
function App() {
  ...
  const filteredWords = useMemo(() => searchWordBank(deferredQuery), [deferredQuery]);
  ...
  function saveWord(word) {
    updateWorkspace((current) => {
      const exists = current.savedWords.some((item) => getWordText(item) === word.word);
...
function Dashboard({ data, setActiveTab, insertText, exportProjectBackup, importProjectBackup }) {
  ...
  return (
    <section className="page-grid">
      <div className="metric-card soft-lilac">
      ...
      <div className="metric-card soft-mint">
        <div className="metric-head">
          <span>最近角色</span>
          <Users size={18} />
        </div>
        ...
        <div className="doc-list">
          {recentDocuments.length ? (
            recentDocuments.map((document) => (
              <button className="doc-row" key={document.id} onClick={() => setActiveTab("editor")}>
      ...
      <div className="panel dashboard-side-panel">
        ...
        <div className="chip-cloud">
          {data.savedWords.length ? (
            savedWordPreview.map((word) => (
              <button className="chip" key={word.id} onClick={() => insertText(getWordText(word))}>
                {getWordText(word)}
              </button>
            ))
          ) : (
...
function EditorPage({
  ...
  return (
    <section className={isChapterPanelOpen ? "editor-layout" : "editor-layout chapter-collapsed"}>
        ...
        <div className="editor-toolbar">
          ...
          <details className="more-actions">
            ...
            <div className="export-button-row">
              ...
              <button className="ghost-button compact" onClick={() => copyText(activeDocument?.content ?? "")} disabled={!activeDocument}>
                <Copy size={16} />
                複製全文
              </button>
            </div>
      ...
      <aside className="tool-pane">
        <div className="tool-section">
          ...
          <ToolShortcut icon={Heart} title="收藏詞語" action={() => insertText(getWordText(data.savedWords[0] ?? {}))} />
        </div>
        <div className="tool-section">
          <span>角色工具</span>
            ...
            data.characters.map((character) => (
              <div className={expandedCharacterIds.has(character.id) ? "quick-character expanded" : "quick-character"} key={character.id}>
                <div className="quick-name-row primary">
                  ...
                  <button title={`插入 ${character.fullName}`} onClick={() => insertText(character.fullName)}>
                    <Plus size={13} />
                  </button>
                </div>
                    ...
                    {character.nicknames.length ? (
                      character.nicknames.map((nickname) => (
                        <div className="quick-name-row sub" key={nickname}>
                          ...
                          <button title={`插入 ${nickname}`} onClick={() => insertText(nickname)}>
                            <Plus size={13} />
                          </button>
                        </div>
                      ))
...
function ToolShortcut({ icon: Icon, title, action }) {
  return (
    <button className="tool-shortcut" onClick={action}>
      <Icon size={18} />
      <span>{title}</span>
    </button>
  );
...
function WordFinder({ query, setQuery, results, savedWords, copyText, insertText, saveWord }) {
  ...
  return (
    <section className="panel full">
      <div className="search-box">
        <Search size={20} />
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="輸入詞語、意思、情緒或語氣，例如：霧、失落、學術、溫柔" />
      </div>
        ...
        {results.map((word) => (
          <article className="word-card" key={word.id}>
            ...
            <div className="button-row">
              <button className="ghost-button compact" onClick={() => copyText(word.word)}>
                <Copy size={15} />
                複製
              </button>
              <button className="ghost-button compact" onClick={() => saveWord(word)} disabled={savedWordTexts.has(word.word)}>
                <Heart size={15} />
                {savedWordTexts.has(word.word) ? "已收藏" : "收藏"}
              </button>
              <button className="primary-button compact" onClick={() => insertText(word.word)}>
                插入文章
              </button>
            </div>
          </article>
        ))}
        {!results.length && <EmptyState title="找不到相關詞語" description="試試輸入更短的字、情緒或語氣。" />}
      </div>
    </section>
  );
}
...
function RelationMapCanvas({
  ...
  }, [nodes, charactersById, connectSourceId, connectTargetId, setFlowNodes]);

  function handleNodeChanges(changes) {
    setFlowNodes((currentNodes) => {
      const nextNodes = applyNodeChanges(changes, currentNodes);
      updateRelationshipNodes(nextNodes);
      return nextNodes;
    });
  }
  ...
  return (
    <section className="panel full relation-flow-panel">
      ...
      <div className="react-flow-canvas">
        ...
        <RelationshipOverlay nodes={flowNodes} relations={relations} viewport={flowViewport} />
      </div>
...
function RelationshipOverlay({ nodes, relations, viewport }) {
...
function Formatter({ mode, setMode, document, characters, applyFormattedDocument }) {
  ...
  function handleAutoFormat() {
    ...
    applyFormattedDocument(formattedDocument);
    setFormatStatus("已根據選項更新文章");
  }
    ...
    <section className="formatter-layout">
      <div className="panel">
        ...
        <div className="format-warning-list">
          <strong>檢查結果</strong>
          {formattedDocument.warnings.length ? (
            formattedDocument.warnings.map((warning) => (
      ...
      <article className={mode === "paper" ? "format-preview paper" : "format-preview novel"} dangerouslySetInnerHTML={{ __html: formattedDocument.html }} />
    </section>
  );
}
...
function getSaveStatusText(status) {
  if (status === "loading") return "正在載入本機資料";
  if (status === "saving") return "正在自動儲存...";
  if (status === "error") return "本機資料保存失敗";
  return "已自動儲存";
}
...
function searchWordBank(query) {
  ...
  return wordBank
    .map((word) => {
      const fields = [
        { value: word.word, weight: 12 },
        { value: word.meaning, weight: 5 },
        { value: word.tags.join(" "), weight: 8 },
        { value: word.tone, weight: 10 },
        { value: word.example, weight: 3 },
      ];
      const score = tokens.reduce((total, token) => {
        ...
        return total + fields.reduce((fieldTotal, field) => {
          ...
          const charHits = tokenChars.filter((char) => value.includes(char)).length;
          return fieldTotal + (charHits / Math.max(tokenChars.length, 1)) * field.weight * 0.45;
        }, 0);
      }, 0);
    ...
    .sort((a, b) => b.score - a.score || a.word.localeCompare(b.word, "zh-Hant"));
}
